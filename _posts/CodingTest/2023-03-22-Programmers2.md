...
* 만약 어디선가 값이 1004가 들어와서 해당 1004에 [+ n / *2 / *3]을 해서 값을 큐에 넣어놨다고 가정해보자.   
연산하다가 갑자기 어느곳에서 1004가 한번더 튀어 나온다면 이 1004 값에 또 다시 [+ n / *2 / *3]을 해서 큐에 넣어봤자 큐 대기줄만 길어질 뿐만 아니라 BFS에서는 먼저 온 값이 반드시 cnt가 낮으므로 연산하는 의미가 없다.
...
* queue에 들어간다는 것은 결국 이미 가득 서있는 대기 줄 가장 뒷부분에 서는 것이다.   
